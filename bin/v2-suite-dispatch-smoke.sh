#!/usr/bin/env bash
# V2 suite-surface UNO dispatch smoke for installed 可圈office.app.
#
# Starts the real app bundle with an isolated profile and a UNO accept socket,
# connects with bundled LibreOfficePython/pyuno, opens Writer/Calc/Impress
# frames, and proves V2 shared entry commands resolve across the main
# Office/WPS-equivalent document surfaces.
set -euo pipefail
...
report="${V2_SUITE_DISPATCH_REPORT:-tmp/v2-suite-dispatch-smoke.md}"
log="${V2_SUITE_DISPATCH_LOG:-tmp/v2-suite-dispatch-smoke.log}"
keep_profile="${V2_SUITE_DISPATCH_KEEP_PROFILE:-0}"
profile=""
soffice_pid=""
port=""
...
usage() {
    cat <<'EOF'
Usage:
  v2-suite-dispatch-smoke.sh [--app <bundle>] [--report <path>] [--log <path>] [--keep-profile]
...
Checks:
  ...
  - Opens live Writer, Calc, and Impress document frames.
  - queryDispatch succeeds for shared V2 entrypoints on every suite surface:
    CommandPalette, CoworkTaskManager, PropertyDeck, and DiffReviewDeck.
  - DispatchHelper executes non-modal PropertyDeck and CommandPalette on every surface.

This is not a visible GUI click-through; it is a live app-session suite-surface dispatch smoke.
EOF
}
...
profile="$(mktemp -d /tmp/kqoffice-suite-dispatch-profile.XXXXXX)"
port="${V2_SUITE_DISPATCH_PORT:-$((22000 + RANDOM % 10000))}"

echo "=== V2 suite dispatch smoke ==="
echo "App bundle: $app"
...
models = []
try:
    for surface, factory_url in surfaces:
        model = desktop.loadComponentFromURL(factory_url, "_blank", 0, (hidden_arg(),))
        if model is None:
            record("FAIL", f"{surface} model", f"{factory_url} returned None")
            continue
        models.append((surface, model))
        record("PASS", f"{surface} model", f"{factory_url} hidden document loaded")

        frame = model.CurrentController.Frame
        if frame is None:
            record("FAIL", f"{surface} frame", "CurrentController.Frame missing")
            continue
        record("PASS", f"{surface} frame", "live document frame acquired")

        for command in query_commands:
            dispatch = frame.queryDispatch(parse_uno_url(command), "_self", 0)
            if dispatch:
                record("PASS", f"{surface} queryDispatch",
                       f"{command} resolved on live {surface} frame")
            else:
                record("FAIL", f"{surface} queryDispatch", f"{command} did not resolve")

        for command in dispatch_commands:
            try:
                result = helper.executeDispatch(frame, command, "_self", 0, ())
                record("PASS", f"{surface} executeDispatch",
                       f"{command} executed; result state={getattr(result, 'State', '<none>')}")
            except Exception as exc:
                record("FAIL", f"{surface} executeDispatch", f"{command} raised {exc!r}")
finally:
    for surface, model in reversed(models):
        try:
            model.close(True)
            record("PASS", f"{surface} model", "hidden document closed")
        except Exception as exc:
            record("FAIL", f"{surface} model", f"close failed: {exc!r}")
    try:
        desktop.terminate()
...
with open(report, "w", encoding="utf-8") as f:
    f.write("# V2 Suite Dispatch Smoke\n\n")
    ...
    f.write("- Scope: live app-bundle UNO bridge + Writer/Calc/Impress entry query/dispatch\n\n")
