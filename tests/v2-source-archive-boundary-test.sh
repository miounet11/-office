...
require_token "vcl/source/uitest/uno/uiobject_uno.cxx" "$batch_dir/W4-select-to-act.paths" "W4 UITest root-cause path"
require_token "sw/qa/uitest/selectToAct/test_select_to_act.py" "$batch_dir/W4-select-to-act.paths" "Writer select-to-act UITest path"
require_token "kqoffice/source/ai/cowork/TaskRunner.cxx" "$batch_dir/W5-cowork.paths" "W5 worker path"
require_token "sfx2/source/sidebar/AIChatReleaseGARuntime.cxx" "$batch_dir/V3-native-ai-workspace.paths" "V3 release GA runtime path"
require_token "sw/Library_sw.mk" "$batch_dir/split-needed.paths" "split-needed sw library path"
