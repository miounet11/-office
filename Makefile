...
$(BUILDDIR)/config_host.mk : $(wildcard \
		...
		$(SRCDIR)/download.lst \
		$(SRCDIR)/sysui/desktop/macosx/*.in \
		$(SRCDIR)/m4/*.m4 \
...
check-makefile-version:
	@if awk 'BEGIN {exit ($(MAKE_VERSION) >= 4.0) ;}' ; then \
		echo; \
		echo "Running with make $(MAKE_VERSION) is likely to cause problems, try with $$GNUMAKE instead."; \
		echo; \
		exit 1; \
	fi
...
# Bootstrap
#
bootstrap: check-if-root check-makefile-version compilerplugins
